...
def computeCost( theta, X, y, reg,nnArchi):
    ...
    # Ouput layer
    z3 = sigmoid(a3)
    y_estimate = z3.T * 0.999999999999999
...
def minibatchGradientDescent(theta, X, y, alpha, numIters, batchSize=15, computeCost=True):
    m = y.shape[0]
    costHistory = np.zeros((numIters, 1))
    ...
    for i in range(numIters):
        ...
        gradient = computeGradient(theta, X_batch, y_batch)
...
# SAVE = True
# LOAD = True
# FILE_TO_LOAD = "04-10_20-58-08_ACC_TR-0.8672_ACC_TE-0.8696_FC-0.3459_REG_5.000_HN1-040_I-03000_para.csv"
# FILE_TO_LOAD = "savedData/" + FILE_TO_LOAD
...
if __name__ == '__main__':
    ...
    # Advance Optimization Algorithms: "BFGS"-"CG"
    startTime1 = time.time()
    res = minimize(computeCost, theta, args=(xTrain, yTrain,REG,nn1), method="CG", \
                   jac=computeGradient, callback=None, options={'disp': True, 'maxiter': NUM_ITERS})
    theta = res.x
    endTime1 = time.time()
